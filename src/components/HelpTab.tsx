...
const CHANGELOG = [
  {
    ...
    label: "update",
    changes: [
      "ひろば：「相談・OB訪問」と「社内活動・PJメンバー募集」の2カテゴリに分割",
      "社内活動ボード：勉強会・読書会・部活・社内PJなどをカテゴリ・頻度つきで登録・閲覧できるように",
      "やりとりタブをシンプルなフィード形式に刷新：返信が必要／確定済み／返信待ちをひとつの画面で確認できるように",
    ],
  },
  {
    version: "v1.0",
    ...
    changes: [
      "アプリ名を「Papple」に変更",
      "プロフィールモーダル：案件経験をクリックで説明文が展開できるように",
      "プロフィールモーダル：前職経歴（prevJobs）をキャリアパスの前に表示",
      "HelpTab：ユーザー向け価値訴求（社内の壁がなくなる・繋がりができる・キャリア不安が和らぐ）をファーストビューに",
      "HelpTab：経営層向けKPI（離職率低下）は下部セクションに移動",
      "全ユーザーの参画案件に詳細エピソードを追加",
    ],
  },
  {
    version: "v0.9",
    ...
    label: "update",
    changes: [
      "いまのきもちステータス機能を追加：飲みたい🍺 / 話したい💬 / ランチしたい🍜 / 話しかけるな🙅",
      "DND状態のユーザーはカードが半透明になり、モーダルのCTAが無効化される",
      "MBTI・FFS第一因子をプロフィールに追加",
    ],
  },
  {
    version: "v0.8",
    ...
    label: "update",
    changes: [
      "どうぶつの森風デザインに全面刷新：M PLUS Rounded 1cフォント、緑パレット、ドット背景、3Dボタン",
      "プロフィールカードをひとこと引用ファーストのデザインに変更（Pitta/Meety風）",
      "絞り込みをアコーディオンから横スクロールチップUIに変更",
    ],
  },
  {
    version: "v0.7",
...
export default function HelpTab() {
  ...
  return (
    <div className="px-4 md:px-8 pt-4 pb-12 flex flex-col gap-10 max-w-3xl mx-auto">
      ...
      {/* ── 1. ユーザーへの価値（ファーストビュー） ── */}
      <section>
        <SectionHeader icon={<Smile size={16} />} label="Papple で変わること" />
